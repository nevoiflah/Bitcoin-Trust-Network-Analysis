...
cells.append(nbf.v4.new_markdown_cell("""# פרויקט מסכם: ניתוח רשת אמון וזיהוי הונאות ב-Bitcoin OTC
...
cells.append(nbf.v4.new_code_cell("%pip install networkx pandas matplotlib seaborn scikit-learn nbformat nbconvert"))

cells.append(nbf.v4.new_markdown_cell("""### 1. טעינת נתונים (Data Loading)
...
cells.append(nbf.v4.new_markdown_cell("""### 2. יצירת גרף חלוקה לרשתות אמון וחוסר-אמון
...
cells.append(nbf.v4.new_code_cell("""# יצירת גרף מכוון
...
cells.append(nbf.v4.new_markdown_cell("""### 3. חקירת נתונים (EDA) וויזואליזציה
...
cells.append(nbf.v4.new_markdown_cell("""### 4. מדדי מרכזיות (Centrality) ואיתור משתמשי מפתח
...
cells.append(nbf.v4.new_code_cell("""# PageRank על רשת אמון
...
cells.append(nbf.v4.new_markdown_cell("""### 5. זיהוי קהילות (Community Detection) באמצעות Louvain ומציאת התארגנויות זדוניות
...
cells.append(nbf.v4.new_code_cell("""from networkx.algorithms.community import louvain_communities
...
cells.append(nbf.v4.new_markdown_cell("""### 6. חיזוי קשרים (Link Prediction) מבוסס זמן
...
cells.append(nbf.v4.new_markdown_cell("""### סיכום ומסקנות (Summary)
...
with open('Bitcoin_Network_Analysis.ipynb', 'w') as f:
    nbf.write(nb, f)
